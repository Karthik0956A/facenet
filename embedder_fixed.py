...
class FaceEmbedder:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            logger.info(f"✓ Model test: input {test_input.shape} → output {test_output.shape}")
    ...
    def preprocess_face(self, face_img: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if len(face.shape) == 3 and face.shape[2] == 3:
                ...
                logger.debug("✓ Converted BGR → RGB")
            elif len(face.shape) == 2:
                # Grayscale - convert to RGB
                face = cv2.cvtColor(face, cv2.COLOR_GRAY2RGB)
                logger.debug("✓ Converted GRAY → RGB")
            ...
            if face.max() > 1.0:
                face = face / 255.0
                logger.debug(f"✓ Normalized to [0, 1]: range=[{face.min():.3f}, {face.max():.3f}]")
            ...
            logger.debug(f"✓ Preprocessed face hash: {face_hash}")
            
            if face.std() < 0.01:
                logger.warning("⚠️  Face has very low variance - might be blank/constant image!")
    ...
    def generate_embedding(self, face_img: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.info(f"✓ Preprocessed: shape={face_processed.shape}, "
            ...
            logger.info(f"✓ Added batch dim: {face_processed.shape} → {face_batch.shape}")
            ...
            logger.info(f"✓ Model output: shape={embeddings_batch.shape}, "
            ...
            if norm < 1e-10:
                logger.error("❌ Embedding has zero norm! Cannot normalize.")
                return None
            
            if np.isclose(norm, 1.0, atol=0.01):
                # Already normalized by model
                embedding_normalized = embedding
                logger.info(f"✓ Already normalized by model (norm={norm:.6f})")
            else:
                # Need to normalize
                embedding_normalized = embedding / norm
                logger.info(f"✓ Normalized embedding (was norm={norm:.6f}, now={np.linalg.norm(embedding_normalized):.6f})")
            ...
            if not np.isclose(final_norm, 1.0, atol=0.01):
                logger.error(f"❌ Embedding not properly normalized! Norm={final_norm:.10f}")
                return None
            
            # Check for degenerate cases
            if embedding_normalized.std() < 0.001:
                logger.warning("⚠️  Embedding has very low variance - might be constant!")
            
            if np.allclose(embedding_normalized, embedding_normalized[0]):
                logger.error("❌ Embedding has all identical values!")
                return None
            
            if np.allclose(embedding_normalized, 0):
                logger.error("❌ Embedding is zero vector!")
                return None
