...
class FaceDatabase:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[OK] Connected to MongoDB: {self.database_name}.{self.collection_name} ({count} faces)")
    ...
    def insert_face(self, face_id: str, name: str, embedding: np.ndarray,
                   summary: Optional[str] = None, metadata: Optional[Dict] = None) -> bool:
        """
        ...
        Args:
            ...
            summary: Optional description/notes about the person
        ...
        try:
            ...
            if not isinstance(embedding, np.ndarray):
                logger.error(f"[ERROR] Embedding is not numpy array: {type(embedding)}")
                return False
            
            if embedding.shape != (512,):
                logger.error(f"[ERROR] Wrong embedding shape: {embedding.shape}")
                return False
            
            if embedding.dtype != np.float32:
                logger.warning(f"[WARN] Embedding dtype is {embedding.dtype}, converting to float32")
            ...
            logger.info(f"[OK] Inserted document: id={result.inserted_id}")
            ...
            if retrieved_hash != embedding_hash:
                logger.error("[ERROR] HASH MISMATCH! Data corrupted during storage!")
            ...
            logger.info("[OK] Hash verified - storage integrity confirmed")
            
            # Check precision loss
            max_diff = np.abs(embedding - retrieved_embedding).max()
            if max_diff > 1e-6:
                logger.warning(f"[WARN] Precision loss detected: max_diff={max_diff:.15f}")
            else:
                logger.info(f"[OK] Precision preserved (max_diff={max_diff:.15f})")
        ...
        try:
            ...
            for i, face in enumerate(faces):
                ...
                if embedding_list is None:
                    logger.warning(f"[WARN] Face {face_id} has no embedding!")
                    continue
                
                if len(embedding_list) != 512:
                    logger.warning(f"[WARN] Face {face_id} has wrong embedding length: {len(embedding_list)}")
                    continue
                ...
                if not np.isclose(norm, 1.0, atol=0.1):
                    logger.warning(f"[WARN] Face {face_id} ({name}) has abnormal norm: {norm:.6f}")
                ...
                if stored_hash and current_hash != stored_hash:
                    logger.warning(f"[WARN] Face {face_id} ({name}) hash mismatch! Data may be corrupted.")
            ...
            logger.info(f"[OK] Validated {len(validated_faces)} embeddings")
    ...
    def get_face_by_name(self, name: str) -> Optional[Dict]:
        """Get single face by name (returns first match). For backward compatibility."""
    ...
    def delete_face(self, face_id: str) -> bool:
        ...
        try:
            ...
            if result.deleted_count > 0:
                logger.info(f"[OK] Deleted face: {face_id}")
                return True
            else:
    ...
    def delete_faces_by_name(self, name: str) -> int:
        ...
        try:
            ...
            logger.info(f"[OK] Deleted {count} faces for {name}")
            return count
    ...
    def clear_all(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[OK] Cleared {count} faces from database")
            return True
...
if __name__ == "__main__":
    ...
    if success:
        ...
        if retrieved:
            ...
            if diff < 1e-6:
                print("[OK] Storage precision preserved")
            else:
                print(f"[ERROR] Precision loss: {diff:.15f}")
